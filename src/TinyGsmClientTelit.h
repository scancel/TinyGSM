...
#ifndef TinyGsmClientTelit_h
#define TinyGsmClientTelit_h
//#pragma message("TinyGSM:  TinyGsmClientTelit")
...
class TinyGsmTelit : public TinyGsmModem
{
...
{
  friend class TinyGsmTelit;
  typedef TinyGsmFifo<uint8_t, TINY_GSM_RX_BUFFER> RxFifo;

public:
  GsmClient() {}

  GsmClient(TinyGsmTelit& modem, uint8_t mux = 1) {
    init(&modem, mux);
  }

  bool init(TinyGsmTelit* modem, uint8_t mux = 1) {
    this->at = modem;
    this->mux = mux;
    sock_available = 0;
    prev_check = 0;
...
public:
  ...
  virtual int available() {
    TINY_GSM_YIELD();
    if (!rx.size()) {
      // Workaround: sometimes Telit forgets to notify about data arrival.
...
public:
  ...
  GsmClientSecure(TinyGsmTelit& modem, uint8_t mux = 1)
    : GsmClient(modem, mux)
  {}
...
public:
  ...
  /*
    During sleep, the Telit module has its serial communication disabled. In order to reestablish communication
    pull the DRT-pin of the Telit module LOW for at least 50ms. Then use this function to disable sleep mode.
  ...
  */
  ...
  bool gprsConnect(const char* apn, const char* user = NULL, const char* pwd = NULL) {    
    ...
    sendAT(GF("#SCFGEXT=1,2,0,1,1,0"));
    if (waitResponse(60000L) != 1)
      return false;

    // Create the socket, and listen on port 80
    sendAT(GF("#SL=1,1,1883,255"));
    if (waitResponse(60000L) != 1)
      return false;
  ...
  bool isGprsConnected() {
    ...
    sendAT(GF("+CGPADDR=1")); // Another option is to use AT+CGPADDR=1
    if (waitResponse() != 1)
      return false;
  ...
  String getGsmLocation() {
    sendAT(GF("$GPSACP"));
    if (waitResponse(10000L, GF(GSM_NL "$GPSACP")) != 1) {
      return "";
    }
    String res = stream.readStringUntil('\n');
    waitResponse();
...
protected:
  ...
  bool modemConnect(const char* host, uint16_t port, uint8_t mux, bool ssl = false) {
    int rsp;
	//sendAT(GF("#SD=2,0,1883,\"demo.budsoft-test.info\",0,0,0"));
	sendAT(GF("#SD=2"), GF(",0,"), port, GF(",\""), host, GF("\",0,0,0"));
  ...
  int16_t modemSend(const void* buff, size_t len, uint8_t mux) {
	DBG("### Buffer to send: ", (char*)buff);
    sendAT(GF("#SSEND=2"), ',', len);
    if (waitResponse(GF(">")) != 1) {
      return 0;
    }
